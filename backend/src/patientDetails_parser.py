...
class PatientDetailsParser(MedicalParser):

    def __init__(self,text):
        MedicalParser.__init__(self,text)
    # parse()->returns the important fields from the text extracted by ocr
    def parse(self):
        return {
            'patient_name':self.get_patient_name(),
            'phone_num':self.get_patient_phone_number(),
            'vaccination_status':self.get_hepatitis_b_vaccination(),
