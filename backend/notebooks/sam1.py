# import function convert_from_path() function to convert the pdf to image
...
list_of_images = convert_from_path(r'C:\Users\ADITHYA GS\OneDrive\Documents\Programs\medicalProject\backend\resources\prescription\pre_2.pdf', poppler_path=r'C:\poppler-23.01.0\Library\bin')
...
# PIL->popular library for image manipulation,opening images etc 

# displays the converted image
# list_of_images[0].show()

# google's ocr engine designed for extracting the text from images

import pytesseract

pytesseract.pytesseract.tesseract_cmd=r"C:\Program Files\Tesseract-OCR\tesseract.exe"

#image_to_string() takes PIL image obj as an input and converts to text and returns a string
# text=pytesseract.image_to_string(list_of_images[0],lang='eng')

# print(type(text))

# print(text)

# But when the image is not clear (if there are dark spots on the image the text in the image might not be recognized)

# Solution:Use image processing and enhance the quality of the image to get better data from the image
...
# pass a rgb image which returns an ndarray
